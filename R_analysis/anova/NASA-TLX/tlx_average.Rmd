...
# ######################### cool box plot (courtesy of Sarah) #########################
# ggplot(task_df,aes(x=auto_level,y=tlx_ave, fill=auto_level))+geom_boxplot(aes(fill=auto_level)) + labs( x = "auto_level", y = "tlx_ave", fill = "auto_level") +   theme_minimal() +
# stat_summary(fun = mean, geom = "point",size = 3, shape = 2, position = position_dodge(0.75), show.legend = FALSE) +
# scale_fill_brewer(palette = "Set2") +
#     theme(plot.title = element_text(size = 15, face = "bold"),
#         axis.title.x = element_text(size = 14, face = "bold"),
...
model <- lmer(tlx_ave ~ auto_num * fitts_id_num + (1 | part_id),
              data = task_df)
summary(model)
