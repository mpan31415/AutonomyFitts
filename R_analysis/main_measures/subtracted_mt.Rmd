...
task_df %>%
  ...
  get_summary_stats(subtracted_mt, type = "mean_sd")
...
bxp1 <- ggboxplot(
  task_df, x = "ring_id", y = "subtracted_mt",
...
task_df %>%
  group_by(auto_level, ring_id) %>%
  identify_outliers(subtracted_mt)

######################### check normality ######################### 
task_df %>%
  group_by(auto_level, ring_id) %>%
  shapiro_test(subtracted_mt)

######################### plot normality ######################### 
ggqqplot(task_df, "subtracted_mt", ggtheme = theme_bw()) +
...
res.aov <- anova_test(
  data = task_df, dv = subtracted_mt, wid = part_id,
...
# one.way <- task_df %>%
#   group_by(ring_id) %>%
#   anova_test(dv = subtracted_mt, wid = part_id, within = auto_level) %>%
#   get_anova_table() %>%
...
# one.way <- task_df %>%
#   group_by(auto_level) %>%
#   anova_test(dv = subtracted_mt, wid = part_id, within = ring_id) %>%
#   get_anova_table() %>%
#   adjust_pvalue(method = "bonferroni")
# one.way
...
pwc1 <- task_df %>%
  ...
  pairwise_t_test(
    subtracted_mt ~ auto_level, paired = TRUE,
...
bxp2 <- ggboxplot(
  task_df, x = "auto_level", y = "subtracted_mt", 
