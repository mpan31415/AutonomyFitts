...
bxp1 <- ggboxplot(
  task_df, x = "fitts_id_level", y = "single_trust",
...
bxp2 <- ggboxplot(
  task_df, x = "auto_level", y = "single_trust", 
...
pwc2 <- task_df %>%
  group_by(auto_level) %>%
  pairwise_t_test(
    single_trust ~ fitts_id_level, paired = TRUE,
    p.adjust.method = "bonferroni"
    )
pwc2
# Create a box plot
...
model <- lmer(single_trust ~ auto_num * fitts_id_num + (1+auto_num|part_id) + (1+fitts_id_num|part_id) + (1|part_id),
              data = task_df)
summary(model)
