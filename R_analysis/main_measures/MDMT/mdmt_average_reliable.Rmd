...
bxp2 <- ggboxplot(
  task_df, x = "auto_level", y = "mdmt_reliable_ave", 
...
pwc2 <- task_df %>%
  group_by(auto_level) %>%
  pairwise_t_test(
    mdmt_reliable_ave ~ fitts_id_level, paired = TRUE,
    p.adjust.method = "bonferroni"
    )
pwc2
# Create a box plot
...
bxp3 <- ggboxplot(
  task_df, x = "fitts_id_level", y = "mdmt_reliable_ave", 
  color = "auto_level", palette = "jco"
) +
  stat_pvalue_manual(
...
model <- lmer(mdmt_reliable_ave ~ auto_num * fitts_id_num + (1+auto_num|part_id) + (1+fitts_id_num|part_id) + (1|part_id),
              data = task_df)
summary(model)
...
interact_plot(fit, pred = fitts_id_num, modx = auto_level, 
              ...
              plot.points = TRUE,
              x.label="Fitts ID", y.label="MDMT Reliable Averaged", 
