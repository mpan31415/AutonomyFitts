...
task_df %>%
  ...
  get_summary_stats(mdmt_skilled, type = "mean_sd")
...
# ######################### cool box plot (courtesy of Sarah) #########################
# ggplot(task_df,aes(x=auto_level,y=mdmt_skilled, fill=auto_level))+geom_boxplot(aes(fill=auto_level)) + labs( x = "auto_level", y = "mdmt_skilled", fill = "auto_level") +   theme_minimal() +
# stat_summary(fun = mean, geom = "point",size = 3, shape = 2, position = position_dodge(0.75), show.legend = FALSE) +
# scale_fill_brewer(palette = "Set2") +
#     theme(plot.title = element_text(size = 15, face = "bold"),
#         axis.title.x = element_text(size = 14, face = "bold"),
...
bxp <- ggboxplot(
  task_df, x = "ring_id", y = "mdmt_skilled",
...
task_df %>%
  group_by(auto_level, ring_id) %>%
  identify_outliers(mdmt_skilled)

######################### check normality ######################### 
task_df %>%
  group_by(auto_level, ring_id) %>%
  shapiro_test(mdmt_skilled)

######################### plot normality ######################### 
ggqqplot(task_df, "mdmt_skilled", ggtheme = theme_bw()) +
...
res.aov <- anova_test(
  data = task_df, dv = mdmt_skilled, wid = part_id,
...
one.way <- task_df %>%
  group_by(ring_id) %>%
  anova_test(dv = mdmt_skilled, wid = part_id, within = auto_level) %>%
  get_anova_table() %>%
...
one.way <- task_df %>%
  group_by(auto_level) %>%
  anova_test(dv = mdmt_skilled, wid = part_id, within = ring_id) %>%
  get_anova_table() %>%
  adjust_pvalue(method = "bonferroni")
one.way
...
pwc <- task_df %>%
  group_by(ring_id) %>%
  pairwise_t_test(
    mdmt_skilled ~ auto_level, paired = TRUE,
...
pwc <- task_df %>%
  group_by(auto_level) %>%
  pairwise_t_test(
    mdmt_skilled ~ ring_id, paired = TRUE,
    p.adjust.method = "bonferroni"
    )
pwc
...
model <- lmer(mdmt_skilled ~ auto_num * fitts_id_num + (1 | part_id),
              data = task_df)
summary(model)
