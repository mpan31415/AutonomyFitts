...
bxp2 <- ggboxplot(
  task_df, x = "auto_level", y = "tlx_mental", 
...
pwc2 <- task_df %>%
  group_by(auto_level) %>%
  pairwise_t_test(
    tlx_mental ~ fitts_id_level, paired = TRUE,
    p.adjust.method = "bonferroni"
    )
pwc2
# Create a box plot
...
bxp3 <- ggboxplot(
  task_df, x = "fitts_id_level", y = "tlx_mental", 
  color = "auto_level", palette = "jco"
) +
  stat_pvalue_manual(
...
model <- lmer(tlx_mental ~ auto_num * fitts_id_num + (1+auto_num|part_id) + (1+fitts_id_num|part_id) + (1|part_id),
              data = task_df)
summary(model)
