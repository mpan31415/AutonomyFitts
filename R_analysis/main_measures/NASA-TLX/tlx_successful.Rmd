...
bxp1 <- ggboxplot(
  task_df, x = "ring_id", y = "tlx_successful",
...
######################### plot normality ######################### 
ggqqplot(task_df, "tlx_successful", ggtheme = theme_bw()) +
...
res.aov <- anova_test(
  data = task_df, dv = tlx_successful, wid = part_id,
...
# one.way <- task_df %>%
#   group_by(ring_id) %>%
#   anova_test(dv = tlx_successful, wid = part_id, within = auto_level) %>%
#   get_anova_table() %>%
...
# one.way <- task_df %>%
#   group_by(auto_level) %>%
#   anova_test(dv = tlx_successful, wid = part_id, within = ring_id) %>%
#   get_anova_table() %>%
#   adjust_pvalue(method = "bonferroni")
# one.way
...
pwc1 <- task_df %>%
  ...
  pairwise_t_test(
    tlx_successful ~ auto_level, paired = TRUE,
...
bxp2 <- ggboxplot(
  task_df, x = "auto_level", y = "tlx_successful", 
...
pwc2 <- task_df %>%
  group_by(auto_level) %>%
  pairwise_t_test(
    tlx_successful ~ fitts_id_level, paired = TRUE,
    p.adjust.method = "bonferroni"
    )
pwc2
# Create a box plot
...
bxp3 <- ggboxplot(
  task_df, x = "fitts_id_level", y = "tlx_successful", 
  color = "auto_level", palette = "jco"
) +
  stat_pvalue_manual(
...
model <- lmer(tlx_successful ~ auto_num * fitts_id_num + (1+auto_num|part_id) + (1+fitts_id_num|part_id) + (1|part_id),
              data = task_df)
summary(model)


###### fit linear mixed model 2 ######
fit <- lm(tlx_successful ~ fitts_id_num * auto_level, data = task_df)
# summary(fit)

interact_plot(fit, pred = fitts_id_num, modx = auto_level, 
              interval = TRUE, int.type = "confidence", int.width = .8,
              plot.points = TRUE,
              x.label="Fitts ID", y.label="NASA-TLX (successful)", 
