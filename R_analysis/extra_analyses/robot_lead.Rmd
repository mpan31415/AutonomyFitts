...
task_df %>%
  group_by(auto_level, ring_id) %>%
  identify_outliers(robot_lead)

######################### check normality ######################### 
task_df %>%
  group_by(auto_level, ring_id) %>%
  shapiro_test(robot_lead)

######################### plot normality ######################### 
ggqqplot(task_df, "robot_lead", ggtheme = theme_bw()) +
...
model <- lmer(robot_lead ~ auto_num * fitts_id_num + (1+auto_num|part_id) + (1+fitts_id_num|part_id) + (1|part_id),
              data = task_df)
summary(model)


###### fit linear mixed model 2 ######
fit <- lm(robot_lead ~ fitts_id_num * auto_level, data = task_df)
# summary(fit)
