...
task_df %>%
  group_by(auto_level, ring_id) %>%
  identify_outliers(arf_human_id)

######################### check normality ######################### 
task_df %>%
  group_by(auto_level, ring_id) %>%
  shapiro_test(arf_human_id)

######################### plot normality ######################### 
ggqqplot(task_df, "arf_human_id", ggtheme = theme_bw()) +
...
res.aov <- anova_test(
  data = task_df, dv = arf_human_id, wid = part_id,
...
# one.way <- task_df %>%
#   group_by(ring_id) %>%
#   anova_test(dv = arf_human_id, wid = part_id, within = auto_level) %>%
#   get_anova_table() %>%
...
# one.way <- task_df %>%
#   group_by(auto_level) %>%
#   anova_test(dv = arf_human_id, wid = part_id, within = ring_id) %>%
#   get_anova_table() %>%
#   adjust_pvalue(method = "bonferroni")
# one.way
...
pwc1 <- task_df %>%
  ...
  pairwise_t_test(
    arf_human_id ~ auto_level, paired = TRUE,
...
bxp2 <- ggboxplot(
  task_df, x = "auto_level", y = "arf_human_id", 
...
pwc2 <- task_df %>%
  group_by(auto_level) %>%
  pairwise_t_test(
    arf_human_id ~ fitts_id_level, paired = TRUE,
    p.adjust.method = "bonferroni"
    )
pwc2
# Create a box plot
...
bxp3 <- ggboxplot(
  task_df, x = "fitts_id_level", y = "arf_human_id", 
  color = "auto_level", palette = "jco"
) +
  stat_pvalue_manual(
