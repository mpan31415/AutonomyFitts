...
task_df %>%
  ...
  get_summary_stats(arf_human_error, type = "mean_sd")
...
bxp1 <- ggboxplot(
  task_df, x = "fitts_id_level", y = "arf_human_error",
...
task_df %>%
  group_by(auto_level, ring_id) %>%
  identify_outliers(arf_human_error)

######################### check normality #########################
task_df %>%
  group_by(auto_level, ring_id) %>%
  shapiro_test(arf_human_error)

######################### plot normality ######################### 
ggqqplot(task_df, "arf_human_error", ggtheme = theme_bw()) +
...
res.aov <- anova_test(
  data = task_df, dv = arf_human_error, wid = part_id,
...
# one.way <- task_df %>%
#   group_by(ring_id) %>%
#   anova_test(dv = arf_human_error, wid = part_id, within = auto_level) %>%
#   get_anova_table() %>%
...
# one.way <- task_df %>%
#   group_by(auto_level) %>%
#   anova_test(dv = arf_human_error, wid = part_id, within = ring_id) %>%
#   get_anova_table() %>%
#   adjust_pvalue(method = "bonferroni")
# one.way
...
pwc1 <- task_df %>%
  ...
  pairwise_t_test(
    arf_human_error ~ auto_level, paired = TRUE,
...
bxp2 <- ggboxplot(
  task_df, x = "auto_level", y = "arf_human_error", 
...
pwc2 <- task_df %>%
  group_by(auto_level) %>%
  pairwise_t_test(
    arf_human_error ~ fitts_id_level, paired = TRUE,
    p.adjust.method = "bonferroni"
    )
pwc2
# Create a box plot
...
model <- lmer(arf_human_error ~ auto_num * fitts_id_num + (1+auto_num|part_id) + (1+fitts_id_num|part_id) + (1|part_id),
              data = task_df)
summary(model)


###### fit linear mixed model 2 ######
fit <- lm(arf_human_error ~ fitts_id_num * auto_level, data = task_df)
# summary(fit)
