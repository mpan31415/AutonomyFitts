...
task_df %>%
  ...
  get_summary_stats(norm_hearf, type = "mean_sd")
...
bxp1 <- ggboxplot(
  task_df, x = "ring_id", y = "norm_hearf",
...
task_df %>%
  group_by(auto_level, ring_id) %>%
  identify_outliers(norm_hearf)

######################### check normality #########################
task_df %>%
  group_by(auto_level, ring_id) %>%
  shapiro_test(norm_hearf)

######################### plot normality ######################### 
ggqqplot(task_df, "norm_hearf", ggtheme = theme_bw()) +
...
res.aov <- anova_test(
  data = task_df, dv = norm_hearf, wid = part_id,
...
# one.way <- task_df %>%
#   group_by(ring_id) %>%
#   anova_test(dv = norm_hearf, wid = part_id, within = auto_level) %>%
#   get_anova_table() %>%
...
# one.way <- task_df %>%
#   group_by(auto_level) %>%
#   anova_test(dv = norm_hearf, wid = part_id, within = ring_id) %>%
#   get_anova_table() %>%
#   adjust_pvalue(method = "bonferroni")
# one.way
...
pwc1 <- task_df %>%
  ...
  pairwise_t_test(
    norm_hearf ~ auto_level, paired = TRUE,
...
bxp2 <- ggboxplot(
  task_df, x = "auto_level", y = "norm_hearf", 
...
pwc2 <- task_df %>%
  group_by(auto_level) %>%
  pairwise_t_test(
    norm_hearf ~ fitts_id_level, paired = TRUE,
    p.adjust.method = "bonferroni"
    )
pwc2
# Create a box plot
...
bxp3 <- ggboxplot(
  task_df, x = "fitts_id_level", y = "norm_hearf", 
  color = "auto_level", palette = "jco"
) +
  stat_pvalue_manual(
...
model <- lmer(norm_hearf ~ auto_num * fitts_id_num + (1+auto_num|part_id) + (1+fitts_id_num|part_id) + (1|part_id),
              data = task_df)
summary(model)


###### fit linear mixed model 2 ######
fit <- lm(norm_hearf ~ fitts_id_num * auto_level, data = task_df)
# summary(fit)
