...
task_df %>%
  ...
  get_summary_stats(human_mean_vel, type = "mean_sd")
...
bxp1 <- ggboxplot(
  task_df, x = "fitts_id_level", y = "human_mean_vel",
...
task_df %>%
  group_by(auto_level, ring_id) %>%
  identify_outliers(human_mean_vel)

######################### check normality ######################### 
task_df %>%
  group_by(auto_level, ring_id) %>%
  shapiro_test(human_mean_vel)

######################### plot normality ######################### 
ggqqplot(task_df, "human_mean_vel", ggtheme = theme_bw()) +
...
res.aov <- anova_test(
  data = task_df, dv = human_mean_vel, wid = part_id,
...
# one.way <- task_df %>%
#   group_by(ring_id) %>%
#   anova_test(dv = human_mean_vel, wid = part_id, within = auto_level) %>%
#   get_anova_table() %>%
...
# one.way <- task_df %>%
#   group_by(auto_level) %>%
#   anova_test(dv = human_mean_vel, wid = part_id, within = ring_id) %>%
#   get_anova_table() %>%
#   adjust_pvalue(method = "bonferroni")
# one.way
...
pwc1 <- task_df %>%
  ...
  pairwise_t_test(
    human_mean_vel ~ auto_level, paired = TRUE,
...
bxp2 <- ggboxplot(
  task_df, x = "auto_level", y = "human_mean_vel", 
...
pwc2 <- task_df %>%
  group_by(auto_level) %>%
  pairwise_t_test(
    human_mean_vel ~ fitts_id_level, paired = TRUE,
    p.adjust.method = "bonferroni"
    )
pwc2
# Create a box plot
...
model <- lmer(human_mean_vel ~ auto_num * fitts_id_num + (1+auto_num|part_id) + (1+fitts_id_num|part_id) + (1|part_id),
              data = task_df)
summary(model)


###### fit linear mixed model 2 ######
fit <- lm(human_mean_vel ~ fitts_id_num * auto_level, data = task_df)
# summary(fit)

interact_plot(fit, pred = fitts_id_num, modx = auto_level, 
              interval = TRUE, int.type = "confidence", int.width = .8,
              plot.points = TRUE,
              x.label="Fitts ID", y.label="Human Mean Velocity (m/s)", 
